...
// --- KEY EXPANSION (256-bit Specific) ---
// 256-bit key = 8 words (nk=8)
// 14 rounds (nr=14)
void KeyExpansion(uint8_t* RoundKey, const uint8_t* Key) {
    ...
    for (i = nk; i < 4 * (nr + 1); ++i) {
        ...
        // AES-256 SPECIAL STEP: Extra S-Box lookup for 8-word keys
        else if (i % nk == 4) {
            tempa[0] = sbox[tempa[0]]; tempa[1] = sbox[tempa[1]];
            tempa[2] = sbox[tempa[2]]; tempa[3] = sbox[tempa[3]];
        }
...
// --- CORE CIPHER (256-bit Specific) ---
void AES256_Encrypt(uint8_t* buffer, const uint8_t* key) {
    uint8_t RoundKey[240]; // 240 bytes for 256-bit key schedule
    KeyExpansion(RoundKey, key);

    AddRoundKey(0, buffer, RoundKey); 

    // 256-bit has 14 Rounds. Loop runs 1 to 13.
    ...
    SubBytes(buffer);
    ShiftRows(buffer);
    AddRoundKey(14, buffer, RoundKey);
}
