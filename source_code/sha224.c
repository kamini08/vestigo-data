...
typedef struct {
    uint8_t data[SHA224_BLOCK_SIZE];
    uint32_t datalen;
    uint64_t bitlen;
    uint32_t state[8];
} SHA224_CTX;
...
static void sha224_transform(SHA224_CTX *ctx, const uint8_t data[]) {
...
void sha224_update(SHA224_CTX *ctx, const uint8_t data[], size_t len) {
    for (size_t i = 0; i < len; ++i) {
        ...
        ctx->datalen++;
        if (ctx->datalen == SHA224_BLOCK_SIZE) {
...
int main() {
    ...
    printf("SHA-224 Hash Function\n");
    printf("Message: %s\n", message);
    
    sha224_init(&ctx);
    sha224_update(&ctx, (uint8_t*)message, strlen(message));
